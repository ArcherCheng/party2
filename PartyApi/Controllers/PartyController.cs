...
namespace PartyApi.Controllers
{
    ...
    public class PartyController : BaseController
    {
        ...
        // [AllowAnonymous]
        // [HttpGet]
        // public async Task<IActionResult> GetList([FromQuery]ParaParty para)
        // {
        //     //取得到今天尚未到期的活動
        //     var partys = await _repo.GetList(para);
        ...
        public async Task<IActionResult> GetActivity([FromQuery]ParaParty para)
        {
            ...
            var repoParty = await _repo.GetActivityList();
            var dtoPartyList = _mapper.Map<IEnumerable<DtoPartyList>>(repoParty);
            return Ok(dtoPartyList);
        }

        [AllowAnonymous]
        [HttpGet("old")]
        public async Task<IActionResult> GetOld([FromQuery]ParaParty para)
        {
            //取得歷史活動
            var partys = await _repo.GetOldList(para);
            var dtoPartyList = _mapper.Map<IEnumerable<DtoPartyList>>(partys);
            Response.AddPagination(partys.CurrentPage,partys.PageSize,partys.TotalCount,partys.TotalPages);
            return Ok(dtoPartyList);
        }
